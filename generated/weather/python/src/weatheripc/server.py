...
class WeatherServer:
    ...
    def __init__(self, connection: IBrokerConnection, instance_id: str, initial_property_values: WeatherInitialPropertyValues, prefix: str):
        ...
        self._method_refresh_daily_forecast_handler = None  # type: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]]
        ...
        self._method_refresh_hourly_forecast_handler = None  # type: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]]
        ...
        self._method_refresh_current_conditions_handler = None  # type: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]]
    ...
    def _receive_location_update_request_message(self, message: Message):
        ...
        try:
            ...
            for location_callback in self._property_location.callbacks:
                ...
                result = location_callback(current_prop_obj)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'location' property, but no event loop was provided to the server to run it on.")
    ...
    def _receive_current_condition_refresh_interval_update_request_message(self, message: Message):
        ...
        try:
            ...
            for current_condition_refresh_interval_callback in self._property_current_condition_refresh_interval.callbacks:
                ...
                result = current_condition_refresh_interval_callback(current_prop_obj.seconds)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(
                            f"Received coroutine result from property update callback for 'current_condition_refresh_interval' property, but no event loop was provided to the server to run it on."
                        )
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def _receive_hourly_forecast_refresh_interval_update_request_message(self, message: Message):
        ...
        try:
            ...
            for hourly_forecast_refresh_interval_callback in self._property_hourly_forecast_refresh_interval.callbacks:
                ...
                result = hourly_forecast_refresh_interval_callback(current_prop_obj.seconds)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(
                            f"Received coroutine result from property update callback for 'hourly_forecast_refresh_interval' property, but no event loop was provided to the server to run it on."
                        )
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def _receive_daily_forecast_refresh_interval_update_request_message(self, message: Message):
        ...
        try:
            ...
            for daily_forecast_refresh_interval_callback in self._property_daily_forecast_refresh_interval.callbacks:
                ...
                result = daily_forecast_refresh_interval_callback(current_prop_obj.seconds)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(
                            f"Received coroutine result from property update callback for 'daily_forecast_refresh_interval' property, but no event loop was provided to the server to run it on."
                        )
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def handle_refresh_daily_forecast(self, handler: Callable[[], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def _process_refresh_daily_forecast_call(self, message: Message):
        ...
        if self._method_refresh_daily_forecast_handler is not None:
            ...
            try:
                return_values = self._method_refresh_daily_forecast_handler(*method_args)
                if inspect.iscoroutine(return_values):
                    if self._event_loop is None:
                        raise RuntimeError(f"Method handler for 'refresh_daily_forecast' returned a coroutine, but no event loop was provided to the server to run it on.")
    ...
    def handle_refresh_hourly_forecast(self, handler: Callable[[], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def _process_refresh_hourly_forecast_call(self, message: Message):
        ...
        if self._method_refresh_hourly_forecast_handler is not None:
            ...
            debug_msg = None  # type: Optional[str]
            try:
                return_values = self._method_refresh_hourly_forecast_handler(*method_args)
                if inspect.iscoroutine(return_values):
                    if self._event_loop is None:
                        raise RuntimeError(f"Method handler for 'refresh_hourly_forecast' returned a coroutine, but no event loop was provided to the server to run it on.")
                    return_values = asyncio.run_coroutine_threadsafe(return_values, self._event_loop).result()
    ...
    def handle_refresh_current_conditions(self, handler: Callable[[], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def _process_refresh_current_conditions_call(self, message: Message):
        ...
        if self._method_refresh_current_conditions_handler is not None:
            ...
            debug_msg = None  # type: Optional[str]
            try:
                return_values = self._method_refresh_current_conditions_handler(*method_args)
                if inspect.iscoroutine(return_values):
                    if self._event_loop is None:
                        raise RuntimeError(f"Method handler for 'refresh_current_conditions' returned a coroutine, but no event loop was provided to the server to run it on.")
                    return_values = asyncio.run_coroutine_threadsafe(return_values, self._event_loop).result()
    ...
    def location(self, value: LocationProperty):
        ...
        value_updated = False
        with self._property_location.mutex:
            if (value != self._property_location.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_location_updated(self, handler: Callable[[float, float], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def current_temperature(self, temperature_f: float):
        ...
        value_updated = False
        with self._property_current_temperature.mutex:
            if (temperature_f != self._property_current_temperature.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def current_condition(self, value: CurrentConditionProperty):
        ...
        value_updated = False
        with self._property_current_condition.mutex:
            if (value != self._property_current_condition.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def daily_forecast(self, value: DailyForecastProperty):
        ...
        value_updated = False
        with self._property_daily_forecast.mutex:
            if (value != self._property_daily_forecast.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def hourly_forecast(self, value: HourlyForecastProperty):
        ...
        value_updated = False
        with self._property_hourly_forecast.mutex:
            if (value != self._property_hourly_forecast.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def current_condition_refresh_interval(self, seconds: int):
        ...
        value_updated = False
        with self._property_current_condition_refresh_interval.mutex:
            if (seconds != self._property_current_condition_refresh_interval.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_current_condition_refresh_interval_updated(self, handler: Callable[[int], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def hourly_forecast_refresh_interval(self, seconds: int):
        ...
        value_updated = False
        with self._property_hourly_forecast_refresh_interval.mutex:
            if (seconds != self._property_hourly_forecast_refresh_interval.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_hourly_forecast_refresh_interval_updated(self, handler: Callable[[int], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def daily_forecast_refresh_interval(self, seconds: int):
        ...
        value_updated = False
        with self._property_daily_forecast_refresh_interval.mutex:
            if (seconds != self._property_daily_forecast_refresh_interval.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_daily_forecast_refresh_interval_updated(self, handler: Callable[[int], Union[None, Coroutine[Any, Any, None]]]):
...
class WeatherServerBuilder:
    ...
    def __init__(self):
        ...
        self._refresh_daily_forecast_method_handler: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]] = None
        self._refresh_hourly_forecast_method_handler: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]] = None
        self._refresh_current_conditions_method_handler: Optional[Callable[[], Union[None, Coroutine[Any, Any, None]]]] = None

        self._location_property_callbacks: List[Callable[[float, float], Union[None, Coroutine[Any, Any, None]]]] = []
        self._current_temperature_property_callbacks: List[Callable[[float], Union[None, Coroutine[Any, Any, None]]]] = []
        self._current_condition_property_callbacks: List[Callable[[WeatherCondition, str], Union[None, Coroutine[Any, Any, None]]]] = []
        self._daily_forecast_property_callbacks: List[Callable[[ForecastForDay, ForecastForDay, ForecastForDay], Union[None, Coroutine[Any, Any, None]]]] = []
        self._hourly_forecast_property_callbacks: List[Callable[[ForecastForHour, ForecastForHour, ForecastForHour, ForecastForHour], Union[None, Coroutine[Any, Any, None]]]] = []
        self._current_condition_refresh_interval_property_callbacks: List[Callable[[int], Union[None, Coroutine[Any, Any, None]]]] = []
        self._hourly_forecast_refresh_interval_property_callbacks: List[Callable[[int], Union[None, Coroutine[Any, Any, None]]]] = []
        self._daily_forecast_refresh_interval_property_callbacks: List[Callable[[int], Union[None, Coroutine[Any, Any, None]]]] = []
