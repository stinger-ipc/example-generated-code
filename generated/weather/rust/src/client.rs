                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_location_prop.recv().await {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!(
                                "No responder provided for property update request for 'location'"
                            );
                        }
                    }
                });
                ...
                tokio::spawn(async move {
                        ...
                        rx_for_current_temperature_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'current_temperature'");
                        }
                    }
                });
            }
                ...
                props.current_condition.take_request_receiver()
            {
                tokio::spawn(async move {
                        ...
                        rx_for_current_condition_prop.recv().await
                    {
                        ...
                        .unwrap();
                        if let Some(responder) = opt_responder {
                            let resp_topic = strfmt("client/{client_id}/weather/property/current_condition/update/response", &topic_param_map).unwrap();
                            let correlation_id = Uuid::new_v4();
                            let (sender, receiver) = oneshot::channel();
                            {
                            ...
                            error!("No responder provided for property update request for 'current_condition'");
                        }
                    }
                });
            }
                ...
                props.daily_forecast.take_request_receiver()
            {
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_daily_forecast_prop.recv().await
                    {
                        ...
                        .unwrap();
                        if let Some(responder) = opt_responder {
                            let resp_topic = strfmt("client/{client_id}/weather/property/daily_forecast/update/response", &topic_param_map).unwrap();
                            let correlation_id = Uuid::new_v4();
                            let (sender, receiver) = oneshot::channel();
                            {
                            ...
                            error!("No responder provided for property update request for 'daily_forecast'");
                        }
                    }
                });
            }
                ...
                props.hourly_forecast.take_request_receiver()
            {
                tokio::spawn(async move {
                        ...
                        rx_for_hourly_forecast_prop.recv().await
                    {
                        ...
                        .unwrap();
                        if let Some(responder) = opt_responder {
                            let resp_topic = strfmt("client/{client_id}/weather/property/hourly_forecast/update/response", &topic_param_map).unwrap();
                            let correlation_id = Uuid::new_v4();
                            let (sender, receiver) = oneshot::channel();
                            {
                            ...
                            error!("No responder provided for property update request for 'hourly_forecast'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                        ...
                        rx_for_current_condition_refresh_interval_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'current_condition_refresh_interval'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                        ...
                        rx_for_hourly_forecast_refresh_interval_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'hourly_forecast_refresh_interval'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                        ...
                        rx_for_daily_forecast_refresh_interval_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'daily_forecast_refresh_interval'");
                        }
                    }
                });
            }
