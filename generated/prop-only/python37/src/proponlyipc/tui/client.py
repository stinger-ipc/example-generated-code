"""Client screen for interacting with PropOnlyIPC server."""
...
from proponlyipc.client import PropOnlyClient
...
class PropertyEditModal(ModalScreen[bool]):
    ...
    def __init__(self, property_name: str, current_value: Any, client: PropOnlyClient):
        super().__init__()
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="property_modal_container"):
            ...
            if self.property_name == "home_address":

                yield TextArea.code_editor(language="json", text=f"{self.current_value.model_dump_json()}", id="property_input")

            if self.property_name == "favorite_country":

                enum_options = [(f"{member.name} ({member.value})", member.value) for member in Country]
        ...
        if event.button.id == "update_button":
            try:
                if self.property_name == "home_address":
                    ...
                    new_home_address_value = Address.model_validate_json(input_widget.value)

                    self.client.home_address = new_home_address_value
                elif self.property_name == "favorite_country":
                    input_widget = self.query_one("#property_input")

                    new_favorite_country_value = Country(input_widget.value)

                    self.client.favorite_country = new_favorite_country_value
            ...
            if self.property_name == "home_address":
                help_text = """The current home address."""
            elif self.property_name == "favorite_country":
                help_text = """The user's favorite country."""
...
class ClientScreen(Screen):
    """Screen for interacting with a connected PropOnlyIPC server."""
    ...
    def on_mount(self) -> None:
        ...
        self.client = self.app.prop_only_client
    ...
    def _register_property_handlers(self) -> None:
        ...
        def register_property(prop_name: str, changed_method_name: str, is_writable: bool = False):
            ...
            if prop_name == "home_address":

                def on_home_address_updated(value: Address):
                    ...
                    if value is None:
                        ...
                        values.append(f"[bold]address.street[/bold]: { value.street }")  # struct member PRIMITIVE

                        values.append(f"[bold]address.city[/bold]: { value.city }")  # struct member PRIMITIVE

                        values.append(f"[bold]address.state[/bold]: { value.state }")  # struct member PRIMITIVE

                        values.append(f"[bold]address.postal_code[/bold]: { value.postal_code }")  # struct member PRIMITIVE

                        values.append(f"[bold]address.country[/bold]: { value.country }")  # struct member ENUM
                ...
                self.client.home_address_changed(on_home_address_updated, call_immediately=True)

            elif prop_name == "favorite_country":

                def on_favorite_country_updated(value: Country):
                    prop_widget.current_value = value
                ...
                # Register the handler with call_immediately=True
                assert self.client is not None, "Client must be initialized"
                self.client.favorite_country_changed(on_favorite_country_updated, call_immediately=True)

        # Register all properties
        register_property("home_address", "home_address_changed", is_writable=True)
        register_property("favorite_country", "favorite_country_changed", is_writable=True)
