# prop-only IPC Library

A Rust library providing client, server, and payload functionality for the prop-only IPC system: Example StingerAPI interface which only has properties, plus a struct and an enum.
...
```bash
# Client example
cargo run --example prop_only_client_demo --features client

# Server example
cargo run --example prop_only_server_demo --features server
```
