"""Discovery screen for finding PropOnlyIPC servers."""
...
from proponlyipc.client import PropOnlyClientDiscoverer, DiscoveredInstance, PropOnlyClient
from proponlyipc.tui.helpers import get_package_version
import logging

logger = logging.getLogger(__name__)

class DiscoveryScreen(Screen):
    """Screen for discovering available PropOnlyIPC servers."""
    ...
    def on_mount(self) -> None:
        ...
        self.discoverer = PropOnlyClientDiscoverer(conn)
    ...
    def _update_grid(self) -> None:
        ...
        if not self.discovered_instances:
            grid.mount(Static("No prop-only instances discovered yet...", classes="no_servers"))
        else:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if hasattr(event.button, 'instance_id'):
            ...
            # Create the PropOnlyClient
            conn = self.app.mqtt_connection
            client = PropOnlyClient(conn, discovered_instance)
            
            # Store the client in the app for the Client screen to use
            self.app.prop_only_client = client
            
            # Create a fresh ClientScreen instance and navigate to it
            from proponlyipc.tui.client import ClientScreen
