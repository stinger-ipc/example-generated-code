...
public:
    std::optional<HomeAddressProperty> homeAddress;
    int homeAddressVersion = 0;

    std::optional<FavoriteCountryProperty> favoriteCountry;
    int favoriteCountryVersion = 0;
...
/**
 * This uses an IConnection object to subscribe to MQTT topics to receive messages that describe the prop-only interface.
 */
class PropOnlyDiscovery {
public:
    // Constructor taking a broker connection and service_id
    PropOnlyDiscovery(std::shared_ptr<stinger::utils::IConnection> broker);

    virtual ~PropOnlyDiscovery();
