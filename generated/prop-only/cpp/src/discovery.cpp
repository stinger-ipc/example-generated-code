...
{
    if (!homeAddress.has_value()) {
        return false;
    }
    if (!favoriteCountry.has_value()) {
        return false;
    }
    return true;
}
...
{
    parent.AddMember("interface_name", rapidjson::Value("prop-only", allocator), allocator);
...
PropOnlyDiscovery::PropOnlyDiscovery(std::shared_ptr<stinger::utils::IConnection> broker):
    _broker(broker)
{
    // Subscribe to the discovery topic
    std::map<std::string, std::string> topicArgs;
    topicArgs["service_id"] = "+";
    topicArgs["interface_name"] = "prop-only";
    topicArgs["client_id"] = "+";
    topicArgs["prefix"] = "+";

    _discoverySubscriptionId = _broker->Subscribe(stinger::utils::format("{prefix}/prop-only/{service_id}/interface", topicArgs), 1);
    _allPropertySubscriptionId = _broker->Subscribe(stinger::utils::format("{prefix}/prop-only/{service_id}/property/+/value", topicArgs), 1);
...
PropOnlyDiscovery::~PropOnlyDiscovery()
{
...
void PropOnlyDiscovery::SetDiscoveryCallback(const std::function<void(const InstanceInfo&)>& cb)
{
    std::lock_guard<std::mutex> lock(_mutex);
    _discovery_callback = cb;
}

std::future<InstanceInfo> PropOnlyDiscovery::GetSingleton()
{
    std::lock_guard<std::mutex> lock(_mutex);
...
std::vector<InstanceInfo> PropOnlyDiscovery::GetInstances() const
{
    std::lock_guard<std::mutex> lock(_mutex);
...
void PropOnlyDiscovery::_onMessage(const stinger::mqtt::Message& msg)
{
    // Check content type
    ...
    if (msg.properties.subscriptionId == _discoverySubscriptionId) {
        ...
        if (infoPtr) {
            if (propertyName == "home_address") {
                auto propertyObj = HomeAddressProperty::FromRapidJsonObject(document);
                infoPtr->initial_property_values.homeAddress = propertyObj;
                infoPtr->initial_property_values.homeAddressVersion = propertyVersion;
            }

            else if (propertyName == "favorite_country") {
                auto propertyObj = FavoriteCountryProperty::FromRapidJsonObject(document);
                infoPtr->initial_property_values.favoriteCountry = propertyObj;
                infoPtr->initial_property_values.favoriteCountryVersion = propertyVersion;
            }
        }
    }
