
#include <iostream>
#include <syslog.h>
#include <thread>
#include <atomic>

#include "server.hpp"
#include "enums.hpp"
#include <stinger/mqtt/brokerconnection.hpp>
#include <stinger/error/return_codes.hpp>

using namespace stinger::gen::prop_only;

int main(int argc, char** argv)
{
    auto conn = std::make_shared<stinger::mqtt::BrokerConnection>("localhost", 1883, "cpp-server-demo");
    conn->SetLogLevel(LOG_DEBUG);
    conn->SetLogFunction([](int level, const char* msg)
                         {
                             std::cout << "[" << level << "] " << msg << std::endl;
                         });

    auto server = std::make_shared<PropOnlyServer>(conn, "cpp-server-demo:1", "example_prefix");

    std::cout << "Setting initial value for property 'home_address'.\n";
    server->updateHomeAddressProperty(Address{ "apples", "apples", "apples", "apples", Country::USA });

    std::cout << "Setting initial value for property 'favorite_country'.\n";
    server->updateFavoriteCountryProperty(Country::USA);

    // Start a background thread that emits signals every 60 seconds.
    std::atomic<bool> keepRunning{ true };
    std::thread periodicEmitter([server, &keepRunning]()
                                {
                                    int loopCount = 0;
                                    while (keepRunning) {
                                        loopCount++;
                                        // Call emitTodayIsSignal; do not block forever waiting for publish
                                        try {
                                        } catch (...) { }

                                        std::cout << "Periodic update iteration " << loopCount << " complete. Sleeping for 60 ...\n";

                                        // Sleep in 1-second increments so we can stop quickly
                                        for (int i = 0; i < 60 && keepRunning; ++i) {
                                            std::this_thread::sleep_for(std::chrono::seconds(1));
                                        }

                                        if (loopCount % 3 == 0) {
                                            std::cout << "Updating value for property 'home_address'.\n";
                                            server->updateHomeAddressProperty(Address{ "apples", "apples", "apples", "apples", Country::USA });

                                            std::cout << "Updating value for property 'favorite_country'.\n";
                                            server->updateFavoriteCountryProperty(Country::USA);
                                        }
                                    }
                                });

    std::cout << "Press ENTER to exit\n";
    std::cin.ignore();

    // Signal the emitter thread to stop and join it
    keepRunning = false;
    if (periodicEmitter.joinable()) {
        periodicEmitter.join();
    }

    return 0;
}