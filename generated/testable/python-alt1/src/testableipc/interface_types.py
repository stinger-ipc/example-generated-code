...
# There are 3 struct definitions
...
class AllTypes(BaseModel):
    ...
    an_entry_object: Annotated[Lunch, Field(description="A struct type.", )]
...
# There are 27 signal definitions
...
class JsonSchemaValidatedIntSignalPayload(BaseModel):
    """ Interface signal `jsonSchemaValidatedInt`. 
    ...
    def validate_schema(self) -> bool:
        ...
        try:
            jsonschema.validate(_data["value"], json.loads(r'{"maximum": 100, "minimum": 0}'))
...
class JsonSchemaValidatedStringSignalPayload(BaseModel):
    """ Interface signal `jsonSchemaValidatedString`. 
    ...
    def validate_schema(self) -> bool:
        ...
        _data = self.model_dump(mode="json", by_alias=True)
        try:
            jsonschema.validate(_data["value"], json.loads(r'{"maxLength": 10, "minLength": 1}'))
        except jsonschema.ValidationError as exc:
            raise ValueError(f"{type(self).__name__} failed JSON schema validation: {exc.message}") from exc
        return True
