                    ...
                    match subscription_id {
                            ...
                            match serde_json::from_slice::<AnotherSignalSignalPayload>(&msg.payload)
                            {
                                Ok(pl) => {
                                    if let Err(e) = pl.validate_schema() {
                                        warn!("Received 'anotherSignal' signal payload failed schema validation: {}", e);
                                        continue;
                                    }
                        ...
                        _i if sub_ids.bark_signal == Some(_i) => {
                            ...
                            match serde_json::from_slice::<BarkSignalPayload>(&msg.payload) {
                                Ok(pl) => {
                                    if let Err(e) = pl.validate_schema() {
                                        warn!("Received 'bark' signal payload failed schema validation: {}", e);
                                        continue;
                                    }
                        ...
                        _i if sub_ids.maybe_number_signal == Some(_i) => {
                            ...
                            match serde_json::from_slice::<MaybeNumberSignalPayload>(&msg.payload) {
                                Ok(pl) => {
                                    if let Err(e) = pl.validate_schema() {
                                        warn!("Received 'maybe_number' signal payload failed schema validation: {}", e);
                                        continue;
                                    }
                        ...
                        _i if sub_ids.maybe_name_signal == Some(_i) => {
                            ...
                            match serde_json::from_slice::<MaybeNameSignalPayload>(&msg.payload) {
                                Ok(pl) => {
                                    if let Err(e) = pl.validate_schema() {
                                        warn!("Received 'maybe_name' signal payload failed schema validation: {}", e);
                                        continue;
                                    }
                        ...
                        _i if sub_ids.now_signal == Some(_i) => {
                            ...
                            match serde_json::from_slice::<NowSignalPayload>(&msg.payload) {
                                Ok(pl) => {
                                    if let Err(e) = pl.validate_schema() {
                                        warn!("Received 'now' signal payload failed schema validation: {}", e);
                                        continue;
                                    }
...
mod tests {
    ...
    async fn mock_client() {
        ...
        let mut client = SignalOnlyClient::new(mock_mqtt.clone(), discovery_info).await;
