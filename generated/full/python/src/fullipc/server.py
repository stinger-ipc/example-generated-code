...
class FullServer:
    ...
    def _receive_favorite_number_update_request_message(self, message: Message):
        ...
        try:
            ...
            for favorite_number_callback in self._property_favorite_number.callbacks:
                ...
                result = favorite_number_callback(current_prop_obj.number)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'favorite_number' property, but no event loop was provided to the server to run it on.")
    ...
    def _receive_favorite_foods_update_request_message(self, message: Message):
        ...
        try:
            ...
            for favorite_foods_callback in self._property_favorite_foods.callbacks:
                ...
                result = favorite_foods_callback(current_prop_obj)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'favorite_foods' property, but no event loop was provided to the server to run it on.")
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def _receive_family_name_update_request_message(self, message: Message):
        ...
        try:
            ...
            for family_name_callback in self._property_family_name.callbacks:
                ...
                result = family_name_callback(current_prop_obj.family_name)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'family_name' property, but no event loop was provided to the server to run it on.")
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def _receive_last_breakfast_time_update_request_message(self, message: Message):
        ...
        try:
            ...
            for last_breakfast_time_callback in self._property_last_breakfast_time.callbacks:
                ...
                result = last_breakfast_time_callback(current_prop_obj.timestamp)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'last_breakfast_time' property, but no event loop was provided to the server to run it on.")
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def _receive_last_birthdays_update_request_message(self, message: Message):
        ...
        try:
            ...
            for last_birthdays_callback in self._property_last_birthdays.callbacks:
                ...
                result = last_birthdays_callback(current_prop_obj)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'last_birthdays' property, but no event loop was provided to the server to run it on.")
                    asyncio.run_coroutine_threadsafe(result, self._event_loop)
    ...
    def on_favorite_number_updated(self, handler: Callable[[int], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def on_favorite_foods_updated(self, handler: Callable[[str, int, Optional[str]], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def lunch_menu(self, value: LunchMenuProperty):
        ...
        value_updated = False
        with self._property_lunch_menu.mutex:
            if (value != self._property_lunch_menu.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_family_name_updated(self, handler: Callable[[str], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def on_last_breakfast_time_updated(self, handler: Callable[[datetime], Union[None, Coroutine[Any, Any, None]]]):
    ...
    def on_last_birthdays_updated(self, handler: Callable[[datetime, datetime, Optional[datetime], Optional[int]], Union[None, Coroutine[Any, Any, None]]]):
...
class FullServerBuilder:
    ...
    def __init__(self):
        ...
        self._favorite_number_property_callbacks: List[Callable[[int], Union[None, Coroutine[Any, Any, None]]]] = []
        self._favorite_foods_property_callbacks: List[Callable[[str, int, Optional[str]], Union[None, Coroutine[Any, Any, None]]]] = []
        self._lunch_menu_property_callbacks: List[Callable[[Lunch, Lunch], Union[None, Coroutine[Any, Any, None]]]] = []
        self._family_name_property_callbacks: List[Callable[[str], Union[None, Coroutine[Any, Any, None]]]] = []
        self._last_breakfast_time_property_callbacks: List[Callable[[datetime], Union[None, Coroutine[Any, Any, None]]]] = []
        self._last_birthdays_property_callbacks: List[Callable[[datetime, datetime, Optional[datetime], Optional[int]], Union[None, Coroutine[Any, Any, None]]]] = []
