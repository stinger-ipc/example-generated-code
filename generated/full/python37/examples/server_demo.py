...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--help":
        ...
        print("Connects to an MQTT broker and demonstrates Full server functionality.")
    ...
    server = FullServer(conn, os.environ.get("SERVICE_ID", "py-server-demo:1"), initial_property_values, prefix=os.environ.get("TOPIC_PARAM_PREFIX", "example"))

    @server.handle_add_numbers
    async def add_numbers(first: int, second: int, third: Optional[int]) -> int:
    ...
    async def do_something(task_to_do: str) -> DoSomethingMethodResponse:
    ...
    async def what_time_is_it() -> datetime:
    ...
    async def hold_temperature(temperature_celsius: float) -> bool:
    ...
    async def on_favorite_number_update(number: int):
    ...
    async def on_favorite_foods_update(drink: str, slices_of_pizza: int, breakfast: Optional[str]):
    ...
    async def on_family_name_update(family_name: str):
    ...
    async def on_last_breakfast_time_update(timestamp: datetime):
    ...
    async def on_last_birthdays_update(mom: datetime, dad: datetime, sister: Optional[datetime], brothers_age: Optional[int]):
