                ...
                tokio::spawn(async move {
                        ...
                        rx_for_favorite_number_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'favorite_number'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_favorite_foods_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'favorite_foods'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_lunch_menu_prop.recv().await {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'lunch_menu'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_family_name_prop.recv().await {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'family_name'");
                        }
                    }
                });
            }
                ...
                props.last_breakfast_time.take_request_receiver()
            {
                tokio::spawn(async move {
                        ...
                        rx_for_last_breakfast_time_prop.recv().await
                    {
                        ...
                        .unwrap();
                        if let Some(responder) = opt_responder {
                            let resp_topic = strfmt("client/{client_id}/Full/property/last_breakfast_time/update/response", &topic_param_map).unwrap();
                            let correlation_id = Uuid::new_v4();
                            let (sender, receiver) = oneshot::channel();
                            {
                            ...
                            error!("No responder provided for property update request for 'last_breakfast_time'");
                        }
                    }
                });
            }
                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_last_birthdays_prop.recv().await
                    {
                        ...
                        if let Some(responder) = opt_responder {
                            ...
                            error!("No responder provided for property update request for 'last_birthdays'");
                        }
                    }
                });
            }
