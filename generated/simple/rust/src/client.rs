...
impl<C: Mqtt5PubSub + Clone + Send + 'static> SimpleClient<C> {
    ...
    pub async fn run_loop(&mut self) -> Result<(), JoinError> {
        ...
        let props = self.properties.clone();
        {
            ...
            let prefix_for_school_prop = self.topic_param_prefix.clone();
            ...
            if let Some(mut rx_for_school_prop) = props.school.take_request_receiver() {
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_school_prop.recv().await {
                        debug!("Property school has changed locally to {:?}, sending update request to server", value);
                        ...
                        let topic_param_map = HashMap::from([
                            ...
                            ("client_id".to_string(), client_id_for_school_prop.clone()),
                            ("prefix".to_string(), prefix_for_school_prop.clone()),
                        ]);
                        ...
                        match message::property_update_request(
                            ...
                            Ok(msg) => {
                                debug!("Publishing property update request for 'school' to topic '{}': {:?}", topic, payload_obj);
                                let _publish_result = publisher_for_school_prop.publish(msg).await;
                                let result = receiver.await;
                                debug!(
                                    "Received property update response for 'school': {:?}",
                                    result
                                );
                                match result {
                                    Ok(MethodReturnCode::Success(opt_response_text)) => {
                                        if let Some(responder) = opt_responder {
                                            ...
                                            info!("No responder provided for property update request for 'school'");
                                        }
                                    }
                                    _ => {
                                        warn!(
                            ...
                            Err(e) => {
                                warn!(
                                    "Failed to create property update message for 'school': {:?}",
                                    e
                                );
                                if let Some(responder) = opt_responder {
                                    let _ = responder.send(None);
