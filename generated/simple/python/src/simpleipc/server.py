...
class SimpleServer:
    ...
    def __init__(self, connection: IBrokerConnection, instance_id: str, initial_property_values: SimpleInitialPropertyValues, prefix: str):
        ...
        self._method_trade_numbers_handler = None  # type: Optional[Callable[[int], Union[int, Coroutine[Any, Any, int]]]]
    ...
    def _receive_school_update_request_message(self, message: Message):
        ...
        try:
            ...
            for school_callback in self._property_school.callbacks:
                ...
                result = school_callback(current_prop_obj.name)

                if inspect.iscoroutine(result):
                    if self._event_loop is None:
                        raise RuntimeError(f"Received coroutine result from property update callback for 'school' property, but no event loop was provided to the server to run it on.")
    ...
    def handle_trade_numbers(self, handler: Callable[[int], Union[int, Coroutine[Any, Any, int]]]):
        ...
        if self._method_trade_numbers_handler is not None:
            ...
            try:
                ...
                if inspect.iscoroutine(return_values):
                    if self._event_loop is None:
                        raise RuntimeError(f"Method handler for 'trade_numbers' returned a coroutine, but no event loop was provided to the server to run it on.")
    ...
    def school(self, name: str):
        ...
        value_updated = False
        with self._property_school.mutex:
            if (name != self._property_school.get_value()) or self._force_property_publish:
                value_updated = True
    ...
    def on_school_updated(self, handler: Callable[[str], Union[None, Coroutine[Any, Any, None]]]):
...
class SimpleServerBuilder:
    ...
    def __init__(self):
        ...
        self._trade_numbers_method_handler: Optional[Callable[[int], Union[int, Coroutine[Any, Any, int]]]] = None

        self._school_property_callbacks: List[Callable[[str], Union[None, Coroutine[Any, Any, None]]]] = []
